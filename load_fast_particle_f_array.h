...
void load_fast_particle_f_array(Particle_f * framework_atoms, Framework framework, Forcefield forcefield, GridParameters parameters) {
	for (int i = 0; i < framework.noatoms; i++) {
		...
		framework_atoms[i].sig_with_adsorbate_squared = pow((parameters.sigma_guest + sigma) / 2.0, 2);
		framework_atoms[i].eps_with_adsorbate = sqrt(parameters.epsilon_guest * epsilon);
