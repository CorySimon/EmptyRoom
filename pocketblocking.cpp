...
inline int energy_ind(int i, int j, int k, int N_x, int N_y, int N_z)
{
...
{
	int i; int j; int k; int global_id;
};
...
void recursively_navigate_connections(int segment_no, vector<Unit_cell_id> & unit_cell_each_segment, bool * segment_visit_status, bool * edge_visit_status, vector<Connection> & connections, int N_segments, int N_connections, bool * channel_found )
{
...
void note_new_connection(int from_segment, int to_segment, int direction, vector<Connection> & connections)
{
	...
	for (int i = 0; i < connections.size(); i++) // look at all present connections to see if it exists already
	{
		if ((to_segment == connections.at(i).to) && (from_segment == connections.at(i).from)) // does a connection between this node already exist?
		{
	...
	if (should_push) // if we should push this connection
	{
		Connection c;
		c.to = to_segment;
		c.from = from_segment;
...
{
	if (parameters.debugmode) printf("Starting blocking pockets routine.\n");
	...
	if (parameters.debugmode) printf("Energy threshold = %f\n",energy_threshold);
	...
	if (parameters.debugmode) printf("Allocated grid point status\nStarting flood fill..\n");
...
//					// get another grid point from the stack
	...
	if (parameters.debugmode) printf("Flood fill done\n");
	if (parameters.debugmode) printf("There are %d segments present.\n", N_segments);
	if (parameters.debugmode) printf("Does it matter if grid includes redundant point i.e. 0 and 1 fractioanl coords?\n");

	if (parameters.debugmode) printf("Checking that all grid points have been found a status\n");
	for (int i = 0 ; i < grid_size; i++)
	{
	...
	if (parameters.debugmode) printf("\tYAY all grid points hv a status now\n");

	//
	// BUILD GRAPH THAT SHOWS HOW SEGMENTS ARE CONNECTED VIA PBC
	//
	// store graph and the direction of their connections
	if (parameters.debugmode) printf("Extracting graph representation of segments\n");
	//
...
//					note_new_connection(this_segment, neighbor_segment, z_dir_neg, connections);
	...
	if (parameters.debugmode) printf("Connections graph created. Number of connections = %d.\n",N_connections);
	...
	for (int seg = 0; seg < N_segments ; seg++)
	{
		...
		if (channel_found)
		{
			if (parameters.debugmode) printf("Found that segment %d is a channel.\n",seg);
			segment_identification.at(seg) = CHANNEL;
		}
		if (! channel_found)
		{
			segment_identification.at(seg) = POCKET;
			if (parameters.debugmode) printf("Didn't find a channel for segment %d.\n",seg);
			segments_that_are_pockets.push_back(seg);
			N_pockets++;
		}
	}
	assert(N_pockets == segments_that_are_pockets.size());
	if (parameters.debugmode) printf("Number of pockets identified = %d\n",N_pockets);

	//
	// FOR EACH POCKET, OVERWRITE ENERGY WITH HIGH VALUE
	//
	if (parameters.debugmode) printf("Overwriting pockets with large energy value\n");
	double high_energy = 1e13;
	for (int pp = 0; pp < N_pockets; pp++)
	{
		if (parameters.debugmode) printf("\tOverwriting pocket %d\n",segments_that_are_pockets.at(pp));
...
void write_cube(string cube_name, Framework framework, Parameters *p, double *energy_grid, int N_x, int N_y, int N_z)
{
	FILE * gridfile;
	char gridfilename[256]="grids/";
	strcat(gridfilename,cube_name.c_str());
	gridfile=fopen(strcat(gridfilename,".cube"),"w");
	fprintf(gridfile,"\nThis is a grid file.\n");
	fprintf(gridfile,"%d % 13.6lf % 13.6lf % 13.6lf\n",0,0.0,0.0,0.0); //enforce zero atoms
	fprintf(gridfile,"%d % 13.6lf % 13.6lf % 13.6lf\n",N_x,p->T_matrix[0][0]/(N_x-1),0.0,0.0);
	fprintf(gridfile,"%d % 13.6lf % 13.6lf % 13.6lf\n",N_y,p->T_matrix[0][1]/(N_y-1),p->T_matrix[1][1]/(N_y-1),0.0);
	fprintf(gridfile,"%d % 13.6lf % 13.6lf % 13.6lf\n",N_z,p->T_matrix[0][2]/(N_z-1),p->T_matrix[1][2]/(N_z-1),p->T_matrix[2][2]/(N_z-1));
	for(int i=0;i<N_x;i++)
	{
		...
		{ // loop over y's
			...
			for(int k=0;k<N_z;k++)
			{
				fprintf(gridfile,"% 13.6E ",energy_grid[energy_ind(i,j,k,N_x,N_y,N_z)]);
				count++;
				if(count==6)
				{
					fprintf(gridfile,"\n");
